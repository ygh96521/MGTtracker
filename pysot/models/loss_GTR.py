...
class MGTtrackerLossComputation(object):
    """
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, labels, reg_targets):
        ...
        if pos_inds.numel() > 0:
            # centerness_targets = self.compute_centerness_targets(labels_flatten)
...
def make_MGTR_loss_evaluator(cfg):
    loss_evaluator = MGTtrackerLossComputation(cfg)
