...
from pysot.models.loss_GTR import make_MGTR_loss_evaluator
from pysot.models.backbone import get_backbone
from pysot.models.head.gtr_head import GTRHead
...
class ModelBuilder(nn.Module):
    def __init__(self):
        ...
        # build car head
        self.gtr_head = GTRHead(cfg, 256)

        # build response map
        self.xcorr_depthwise = xcorr_depthwise

        # build loss
        self.loss_evaluator = make_MGTR_loss_evaluator(cfg)
    ...
    def track(self, x):
        ...
        cls, loc, cen = self.gtr_head(features)
        return {
                'cls': cls,
                'loc': loc,
                'cen': cen
